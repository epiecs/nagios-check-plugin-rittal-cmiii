...
$checkRittal->check(getopt("hD:C:H:W:M:V:", array("help")));
...
class checkRittal
{
    ...
    private function checkUnitStatus($status)
    {
        switch($status)
        {
            case 4:
    ...
    private function checkUnitOutputWattage($wattage, $warningWattage, $maxWattage)
    {
        switch (true)
        {
    ...
    private function checkUnitKwh($kwh)
    {
            $kwh = $kwh/10;
    ...
    private function checkPhaseStatus($phase, $status)
    {
        switch($status)
        {
            case 4:
    ...
    private function checkPhaseInputVoltage($phase, $voltage, $meanVoltage)
    {
        $voltage = $voltage / 10;
        ...
        switch (true)
        {
                ...
                echo "{$phase} {$voltage}V|'Voltage'={$voltage};;;0;250";
                exit(self::STATE_OK);
    ...
    private function checkPhaseOutputWattage($phase, $wattage, $warningWattage, $maxWattage)
    {
        switch (true)
        {
                case $wattage < $warningWattage:
